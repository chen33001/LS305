...
use_vb <- c("SEX", "AGE", 
            ...
            "BUN","CREATININE","URIC_ACID","MICROALB","CREATININE_URINE")
use_meaning<- c("性別", "年齡", 
                "年齡層","身高","體重","身體質量指數","體脂肪率","腰圍","臀圍","腰臀比",
                "靜坐時第一次收縮壓","靜坐時第一次舒張壓",
                "靜坐時第二次收縮壓","靜坐時第二次舒張壓",
                "靜坐時第三次收縮壓","靜坐時第三次舒張壓",
                "靜坐時第一次心跳","靜坐時第二次心跳","靜坐時第三次心跳",
                "STIFFNESS INDEX骨硬度指數","T參數值","Z參數值","超音波速度","寬頻超音波遞減值",
                "肺活量","肺活量基準值","一回換氣量","預備呼氣量","預備吸氣量",
                "預備呼氣量+預備吸氣量","肺活量/身長","強制呼出時肺活量（努力性肺活量）","強制呼出時肺活量基準值",
                "Zeor點至1秒經過時的呼出量（一秒量）","一秒量基準值","1秒率，(FEV1.0/FVC)*100","FEV10_FVC基準值","1秒率，(FEV1.0/VC)*100",
                "FEV10_SVC基準值","FEV10_SVC基準值","(FEV1.0/VC_PRED)*100","FVC的75％到25％的平均量（最大中間呼氣量）","MMF基準值",
                "最大呼氣流量","最大呼氣流量基準值","努力性肺活量75％時流量","FEF25基準值","努力性肺活量50％時流量","FEF50基準值",
                "努力性肺活量25％時流量","FEF75基準值","FEF75/身長","FEF75_HT基準值",
                "(Extrapolated Volume/FVC)*100","FIV1.0/FVC",
                "紅血球","白血球","血小板","血紅素","血球比容","醣化血色素值",
                "C型肝炎抗體","C型肝炎抗體（值）","B型肝炎表面抗原","B型肝炎表面抗原（值）","B型肝炎e抗原","B型肝炎e抗原(值)",
                "B型肝炎表面抗體","B型肝炎表面抗體（值）","B型肝炎核心抗體","B型肝炎核心抗體（值）","D型肝炎抗體","D型肝炎抗體（值）",
                "飯前血糖","總膽固醇","三酸甘油酯","高密度酯蛋白膽固醇","低密度酯蛋白膽固醇",
                "總膽紅素","白蛋白","血清麩胺酸苯醋酸轉氨基酵素","血清麩胺酸丙酮酸轉氨基酵素","A58	γ －麩胺醯轉移酵素","甲型胎兒血清蛋白",
                "血中尿素氮","肌酸酐","尿酸","尿中微蛋白","尿中肌酸酐")
result_set <- data.frame(vb1 = NA, vb2 = NA, vb3 = NA, p_value = NA)
xxx1 <- chiqTCM
...
#匯出檔案---------------------------------------------------------------------------------------------------------------------------------
write.csv(result_set,file='C:\\R\\LS305中醫\\卡方結果.csv',fileEncoding = "Big5",append = FALSE)
